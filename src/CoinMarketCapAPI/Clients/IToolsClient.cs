...
{
    /// <summary>
    ///     Convert an amount of one currency into up to 32 other cryptocurrency or fiat currencies at the same time using
    ///     latest exchange rates. Optionally pass a historical timestamp to convert values based on historic averages.
    ...
    /// <param name="symbol">
    ///     Alternatively the cryptocurrency symbol of the base currency to convert from. Example: "BTC". One
    ///     "id" or "symbol" is required
    /// </param>
    /// <param name="time">
    ///     Optional timestamp (Unix or ISO 8601) to reference historical pricing during conversion. If not
    ///     passed, the current time will be used. If passed, we'll reference the closest historic values available during
    ///     conversion
    /// </param>
    /// <param name="convert">
